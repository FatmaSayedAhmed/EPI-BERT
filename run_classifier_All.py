...
tf.app.flags.DEFINE_string('train_data_name', 'All', '导入的数据名')
tf.app.flags.DEFINE_string('test_data_name', 'GM12878', '导入的数据名')
...
tf.app.flags.DEFINE_string(
    'train_data_root', './DatasetAll/asTF_Record/1kmer_tfrecord/', '使用数据集的根目录')

tf.app.flags.DEFINE_string(
    'test_data_root', './Dataset/asTF_Record/1kmer_tfrecord/', '使用数据集的根目录')
...
tf.app.flags.DEFINE_string(
    'save_path', "./model/Classifier_model_128_All/model_EPI_All_GM12878_100.ckpt", '模型保存位置')
...
def main():
    ...
    train_data_name = FLAGS.train_data_name  # 选定数据名，用于导入对应的路径的数据
    test_data_name = FLAGS.test_data_name  # 选定数据名，用于导入对应的路径的数据

    #Dataset
    train_dict = { "All": 355970,
                  "GM12878": 76020,
                  "HeLa-S3": 62640,
                  "HUVEC": 54800,
                  "IMR90": 45080,
    ...
    train_example_num = train_dict[train_data_name]   # 获取训练集样本数量
    test_example_num = test_dict[test_data_name]     # 获取测试集样本数量
    ...
    train_data_root = FLAGS.train_data_root    # 根据实际调用的数据集来这里修改数据路径
    test_data_root = FLAGS.test_data_root    # 根据实际调用的数据集来这里修改数据路径
    ...
    input_file = train_data_root +  "train_All.tf_record"
    ...
    if do_eval:
        input_file = test_data_root + test_data_name + "/" + test_data_name + "_dev.tf_record"
